...
- サイトはすでに **Vercel** にデプロイしてあり、**本番の URL**（例：`https://jca-2026.vercel.app` や独自ドメイン）で公開されていること。
...
4. 次の変数を **1つずつ** 追加する。  
   ...
   | `NEXT_PUBLIC_SITE_URL` | 本番の URL | 例：`https://あなたのドメイン` |
...
1. **「エンドポイント URL」**（Endpoint URL）の欄に、次の形式で入力する。

   ```
   https://あなたの本番ドメイン/api/webhooks/stripe
   ```

   **例：**

   - Vercel のデフォルト URL の場合：  
     `https://jca-2026.vercel.app/api/webhooks/stripe`
   - 独自ドメインの場合：  
     `https://jp-clarinet.org/api/webhooks/stripe`

   ※ `https://` まで含め、末尾は `/api/webhooks/stripe` にすること。

2. **「リスニングするイベントを選択」**（Select events to listen to）をクリックし、次のイベントにチェックを入れる。

   - **`checkout.session.completed`**  
     （チェックアウトが完了したとき。入会決済完了でメールを送るために必須）

3. **「エンドポイントを追加」** をクリックして保存する。

### 2-4. 「署名シークレット」をコピーして Vercel に入れる
...
| エンドポイント URL | `https://あなたの本番ドメイン/api/webhooks/stripe` |
