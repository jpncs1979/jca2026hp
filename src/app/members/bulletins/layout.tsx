...
export default async function MemberBulletinsLayout({
  children,
}: {
  ...
  if (!auth.ok && auth.reason === "unauthenticated") {
    redirect("/mypage?redirect=/members/bulletins");
  }
    ...
    <div className="font-soft min-h-screen bg-muted/30">
      <header className="sticky top-0 z-40 border-b border-border bg-white">
        <div className="container mx-auto flex h-14 items-center justify-between px-4">
          <Link href="/members/bulletins" className="font-semibold text-navy">
            会員専用ページ
          </Link>
